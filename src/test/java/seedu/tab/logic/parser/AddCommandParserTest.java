...
public class AddCommandParserTest {
    ...
    public void parse_repeatedNonTagValue_failure() {
        String validExpectedStudentString = NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + TAG_DESC_FRIEND;
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, INVALID_NAME_DESC + INVALID_PHONE_DESC + EMAIL_DESC_BOB,
                Messages.getErrorMessageForInvalidValue(Name.FIELD_NAME, "---", Name.getFailureReason("---"))
                        + "\n"
                        + Messages.getErrorMessageForInvalidValue(Phone.FIELD_NAME, "12",
    ...
    public void parse_nameWithSlash_succeeds() {
        ...
        assertParseSuccess(parser, " " + PREFIX_NAME + "Ravi s/o Kumaran" + PHONE_DESC_BOB
                + EMAIL_DESC_BOB, new AddCommand(expected));
        ...
        assertParseSuccess(parser, " " + PREFIX_NAME + "Anita d/o Rajan" + PHONE_DESC_BOB
                + EMAIL_DESC_BOB, new AddCommand(daughter));
    }

    @Test
    public void parse_malaysianNameWithSlash_succeeds() {
        // A/L and A/P are ordinary components of a Malaysian name. They used to be read as the
        // address prefix, which is gone, so the tokenizer no longer takes them for a field.
        Student son = new StudentBuilder().withName("Abdul a/l Rahman").withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withTags().build();
        assertParseSuccess(parser, " " + PREFIX_NAME + "Abdul a/l Rahman" + PHONE_DESC_BOB
                + EMAIL_DESC_BOB, new AddCommand(son));

        Student daughter = new StudentBuilder().withName("Siti a/p Ahmad").withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withTags().build();
        assertParseSuccess(parser, " " + PREFIX_NAME + "Siti a/p Ahmad" + PHONE_DESC_BOB
                + EMAIL_DESC_BOB, new AddCommand(daughter));
    }
    ...
    public void parse_severalInvalidValues_reportsEveryOne() {
        ...
        assertParseFailure(parser, " " + PREFIX_NAME + "---" + " " + PREFIX_PHONE + "12"
                + " " + PREFIX_EMAIL + "john@x" + " " + PREFIX_TAG + "---", expected);
    }
